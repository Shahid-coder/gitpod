...
export async function build(context, version) {
    ...
    exec(`leeway build --werft=true --coverage-output-path=${coverageOutput} -Dversion=${version} -DremoveSources=false -DimageRepoBase=${imageRepo} -DlocalAppVersion=${localAppVersion} -DnpmPublishTrigger=${publishToNpm ? Date.now() : 'false'}`);
    if (publishRelease) {
        try {
    ...
    exec(`curl -s https://codecov.io/bash | bash -s - -s ${coverageOutput}`)
    werft.done('coverage');
